...
5. **Stop and ask** when a build/test fails without an obvious fix (→ agent-skills:debugging-and-error-recovery) or the spec is ambiguous. Re-invoking `/build auto` resumes from the next pending task.
